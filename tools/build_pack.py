#!/usr/bin/env python3
"""Build the French pack with the shared builder in vocab-engine.

Equivalent to:
    PYTHONPATH=engine/tools python3 -m packbuilder build --lang fr --repo . [--stage ...]

The pipeline lives in engine/tools/packbuilder (core stages) and
engine/tools/packbuilder/langs/fr.py (French rules). This repo keeps only the
data it owns: tools/gloss_overrides.json, tools/forced_a1.txt, tools/h_aspire.txt,
and the generated pack/ + tools/REPORT.md.
...
if __name__ == "__main__":
    sys.exit(main(["build", "--lang", "fr", "--repo", str(ROOT)] + sys.argv[1:]))
